...
def run_rank0(args, device, splits):
    ...
    # 1. Initialize
    enable_offload = getattr(args, 'enable_offload', False)
    full = load_stage_model(args.model, device, role="stage0", end=splits[0], dtype=args.dtype, enable_offload=enable_offload)
    if enable_offload:
        s0 = Stage0(full, splits[0], enable_offload=True, gpu_device=device)  # CPU에 유지, forward 시 GPU로 이동
    else:
        s0 = Stage0(full, splits[0], enable_offload=False, gpu_device=device).to(device)  # GPU에 직접 로드
...
def run_stage_server(args, device, splits):
    ...
    if args.stage == 1:
        ...
        full = load_stage_model(args.model, device, role="segment", start=start, end=end, dtype=args.dtype, enable_offload=enable_offload)
        if enable_offload:
            stage_model = StageSegment(full, start, end, enable_offload=True, gpu_device=device)  # CPU에 유지
        else:
            stage_model = StageSegment(full, start, end, enable_offload=False, gpu_device=device).to(device)  # GPU에 직접 로드
        final_stage = False
    elif args.stage == 2:
        ...
        start = splits[2]
        full = load_stage_model(args.model, device, role="last", start=start, dtype=args.dtype, enable_offload=enable_offload)
        if enable_offload:
            stage_model = StageLast(full, start, enable_offload=True, gpu_device=device)  # CPU에 유지
        else:
            stage_model = StageLast(full, start, enable_offload=False, gpu_device=device).to(device)  # GPU에 직접 로드
        final_stage = True
    else:
...
def main():
    ...
    parser.add_argument('--enable_offload', action='store_true', 
                       help='Enable CPU off-loading: model parameters stored in CPU/RAM, moved to GPU only during forward pass')
